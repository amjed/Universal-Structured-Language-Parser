		...
		extends CompositeClause
{
	...
	@Override
	public void addSubClause( Clause subClause, ClauseAlias alias, boolean optional,
							  int maxCardinality ) {
	...
	protected void buildMatchTree( AliasTreeNode parentAlias, VariableContext variableContext,
								   NodeAliasList prevNodes, NodeAliasList ends, boolean last ) {
		NodeAliasList subNextNodes = new NodeAliasList( subclauses.size() );

		for ( int i = 0; i < subclauses.size(); ++i ) {
			SubclauseHolder holder = subclauses.get( i );

			AliasTreeNode aliasTreeNode = computeChildAlias( parentAlias, holder.alias );

			subNextNodes.clear();
			holder.subclause.buildMatchTree( aliasTreeNode, variableContext, prevNodes, subNextNodes,
											 last );
